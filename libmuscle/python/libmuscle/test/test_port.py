...
def test_create_port():
    port = Port("out", Operator.O_I, None, False, True, 0, [])
...
def test_create_port_with_timeline():
    timeline = Timeline(":t")
    port = Port("out", Operator.O_I, timeline, False, True, 0, [])
    assert port.timeline == timeline


def test_create_invalid_port():
    with pytest.raises(RuntimeError):
        Port("out", Operator.O_I, None, True, True, 1, [])

    with pytest.raises(RuntimeError):
        Port("out", Operator.O_F, None, True, True, 0, [10, 10])

    with pytest.raises(RuntimeError):
        Port("out", Operator.O_F, None, False, True, 1, [11, 12])

    with pytest.raises(RuntimeError):
        Port("out", Operator.O_I, None, False, True, 2, [])


def test_port_properties():
    port = Port("out", Operator.O_F, None, False, False, 0, [])
    ...
    port = Port("out", Operator.O_F, None, True, True, 0, [10])
    ...
    port = Port("out", Operator.O_F, None, False, True, 1, [10])
    assert port.is_vector() is False
    assert port.is_resizable() is False
    with pytest.raises(RuntimeError):
        port.get_length()
    ...
    port = Port("out", Operator.O_F, None, True, True, 1, [10, 20])
    assert port.is_vector() is True
    assert port.is_resizable() is False
    ...
    port = Port("out", Operator.O_F, None, False, True, 2, [12])
    assert port.is_vector() is False
    assert port.is_resizable() is False
    with pytest.raises(RuntimeError):
        port.get_length()
    ...
    port = Port("out", Operator.O_F, None, True, True, 1, [13])
    assert port.is_vector() is True
