...
class Communicator:
    ...
    def set_peer_info(self, peer_info: PeerInfo) -> None:
        ...
        for port in self._port_manager.get_connected_ports(Operator.F_INIT):
            ...
            # Sending component handles reducer filters:
        ...
        for port in self._port_manager.get_connected_ports(Operator.S):
            ...
            if any(filter.is_repeater() for filter in filters):
                raise NotImplementedError(
                    ...
                    "the conduit to a F_INIT port instead."
                )
    ...
    def pre_receive_f_init(self) -> FInitCacheType:
        ...
        # Handle messages from repeater ports
        ...
        for port_name in self._f_init_repeaters:
            ...
            while (port_name, slot) not in self._f_init_repeat_cache:
                ...
                for recv_slot in _yield_slots(port):
                    msg = self._receive_message(port_name, recv_slot)
                    if is_subiteration(cur_iteration, msg.iteration):
                        self._f_init_repeat_cache[(port_name, recv_slot)] = msg
                    elif msg.iteration >= cur_iteration:
                        raise RuntimeError(
                            "Got a message from the future on "
                            f"{port_desc(port_name, slot)}. Message iteration is "
                            f"{msg.iteration}, while our iteration is {cur_iteration}."
                        )
            ...
            for slot in _yield_slots(port):
                msg = self._f_init_repeat_cache[(port_name, slot)]
                if not is_subiteration(cur_iteration, msg.iteration):
                    raise RuntimeError(
                ...
                message = _make_message(msg, copy=True)
                if pad_message:
                    message.data = None
