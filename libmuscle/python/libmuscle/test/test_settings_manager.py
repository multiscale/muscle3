...
def test_get_parameter(settings_manager):
    ref = Reference
    settings_manager.base[ref('test')] = 13
    assert settings_manager.get_parameter(ref('instance'), ref('test')) == 13

    settings_manager.overlay[ref('test2')] = 14
    assert settings_manager.get_parameter(ref('instance'), ref('test2')) == 14

    settings_manager.base[ref('test2')] = 'test'
    assert settings_manager.get_parameter(ref('instance'), ref('test2')) == 14

    settings_manager.overlay = Settings()
    assert settings_manager.get_parameter(ref('instance'), ref('test2')) == \
        'test'

    settings_manager.base[ref('test3')] = 'base_test3'
    settings_manager.base[ref('instance.test3')] = 'base_instance_test3'
    assert settings_manager.get_parameter(ref('instance'), ref('test3')) == \
        'base_instance_test3'
    assert settings_manager.get_parameter(ref('instance2'), ref('test3')) == \
        'base_test3'
    ...
    assert settings_manager.get_parameter(ref('instance'), ref('test4')) == \
        'base_test4'

    assert settings_manager.get_parameter(ref('instance[10]'), ref('test4')
                                          ) == 'base_test4'

    settings_manager.base[ref('instance[10].test5')] = 'base_test5'
    settings_manager.overlay[ref('test5')] = 'overlay_test5'
    assert settings_manager.get_parameter(ref('instance'), ref('test5')) == \
        'overlay_test5'
    assert settings_manager.get_parameter(ref('instance[10]'), ref('test5')
                                          ) == 'base_test5'
    assert settings_manager.get_parameter(ref('instance[11]'), ref('test5')
                                          ) == 'overlay_test5'
