program reaction
    ...
    do while (LIBMUSCLE_Instance_reuse_instance(instance))
        ...
        if (LIBMUSCLE_Instance_resuming(instance)) then
            ...
            if (.not. LIBMUSCLE_DataConstRef_is_nil(rdata)) then
                ...
                allocate (U(LIBMUSCLE_DataConstRef_size(item)))
                call LIBMUSCLE_DataConstRef_elements(item, U)
                ...
                t_max = LIBMUSCLE_DataConstRef_as_real8(item)
                call LIBMUSCLE_DataConstRef_free(item)
            end if
